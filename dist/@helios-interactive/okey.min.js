...
!function(exports, global) {
    ...
    function Element(Keyboard, el) {
        ...
        this.onEvent = function(keyInfo) {
            dispatchEvent("keydown", keyInfo), dispatchEvent("keypress", keyInfo), dispatchEvent("keyup", keyInfo), 
            dispatchEvent("input", keyInfo), dispatchEvent("change", keyInfo);
        }, this.keydownfunc = function(e) {
            if (e.virtual) {
                e.preventDefault();
                var selectionSupported = checkSelectionSupport(this.type);
                if ("backspace" != e.key) if ("toggle" != e.key.substring(0, 6)) {
                    ...
                    if (newType.length >= 0 && global.Keyboard.layout[newType]) return this.Keyboard.Keyboard.show(newType);
    ...
    function Keyboard(inputs, holder) {
        ...
        this.keyboardEl.classList.add("keyboard-container"), this.keyboardEl.addEventListener("mousedown", handleKeydownEvents.bind(this)), 
        this.keyboardEl.addEventListener("mouseup", handleKeyupEvents.bind(this)), this.keyboardEl.addEventListener("touchstart", handleKeydownEvents.bind(this)), 
        this.keyboardEl.addEventListener("touchend", handleKeyupEvents.bind(this)), this.show = function(layout) {
            ...
            if (this.clearKeyHoldTimers(), !global.Keyboard.layout[layout]) throw new Error("keyboard initiation: Missing layout: " + layout);
            if (!self.layout || layout !== self.layout || !this.active) {
                ...
                this.keyboardEl.appendChild(closeButton), closeButton.onclick = function() {
                    ...
                    document.activeElement.blur();
                }.bind(this), global.Keyboard.layout[self.layout].forEach(foreachLayout), dispatchKeyboardOpenEvent(), 
                holder.appendChild(self.keyboardEl);
            }
        }, this.hide = function() {
            self.active = !1, this.clearKeyHoldTimers(), dispatchKeyboardCloseEvent(), setTimeout(function() {
            ...
            this.keyHoldTimeout && window.clearTimeout(this.keyHoldTimeout), this.keyHoldInterval && window.clearInterval(this.keyHoldInterval);
        };
    }
    ...
    var handleKeydownEvents = function(e) {
        ...
        if (e.preventDefault(), e.target.classList.contains("keyboard-key")) {
            ...
            self.listeners.key.forEach(function(action) {
                action(keyInfo);
            }), self.clearKeyHoldTimers(), self.keyHoldTimeout = window.setTimeout(function() {
    ...
    }, handleKeyupEvents = function(e) {
        e.target.classList.remove("active"), this.clearKeyHoldTimers();
    };
